...
type ToolView = 'cleanup' | 'json' | 'data-lab' | 'stats' | 'radix' | 'bytes' | 'crypto' | 'diff' | 'convert' | 'color' | 'image-compress' | 'image-crop' | 'background-remove' | 'screen-color' | 'qrcode' | 'ip-check' | 'network-diagnosis' | 'ports' | 'volta' | 'nvm' | 'assistant-prompt'
...
const categories: Array<{ id: ToolCategory; label: string }> = [{ id: 'all', label: '全部工具' }, { id: 'file', label: '文件工具' }, { id: 'data', label: '数据工具' }, { id: 'design', label: '设计工具' }, { id: 'network', label: '网络工具' }]
...
const activeCategory = computed<ToolCategory>(() => {
  ...
  return category === 'file' || category === 'data' || category === 'network' || category === 'design' || category === 'all' ? category : initialCategory
})
